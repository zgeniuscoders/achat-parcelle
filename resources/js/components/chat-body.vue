<template>
    <div id="messages" v-for="message in messages" :key="message.id"
        ...
        <div class="chat-message" v-if="message.sender_id === props.currentUser">
        ...
        <div class="chat-message" v-else>
            <div class="flex items-end justify-end">
                <div class="flex flex-col space-y-2 text-xs max-w-xs mx-2 order-1 items-end">
                    <div><span class="px-4 py-2 rounded-lg inline-block rounded-br-none bg-emerald-600 text-white ">{{ message.message }}</span></div>
                </div>
